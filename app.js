(function(){
 ...
 async function renderHome(){
  ...
  <div class="card"><h2>Les prochains matchs</h2>${matches.filter(m=>{const deadline=m.ticketing_opens_at?new Date(m.ticketing_opens_at):null;const expired=deadline&&new Date()>=deadline;return !expired||attMap[m.id]==='yes';}).length?matches.filter(m=>{const deadline=m.ticketing_opens_at?new Date(m.ticketing_opens_at):null;const expired=deadline&&new Date()>=deadline;return !expired||attMap[m.id]==='yes';}).map(m=>{const deadline=m.ticketing_opens_at?new Date(m.ticketing_opens_at):null;const expired=deadline&&new Date()>=deadline;const ticket=tickets.find(t=>t.match_id===m.id);return `<div class="match"><h3>${esc(m.name)}</h3><div class="small">${esc(m.match_starts_at?new Date(m.match_starts_at).toLocaleString('fr-FR',{dateStyle:'short',timeStyle:'short'}):m.date||'')} · ${esc(m.venue||m.stadium||'Stade non renseigné')}${m.opponent?' · '+esc(m.opponent):''}</div>${!expired?`<div class="actions"><button class="btn yes ${attMap[m.id]==='yes'?'selected':''}" onclick="answer('${m.id}','yes')">Je veux y aller</button><button class="btn no ${attMap[m.id]==='no'?'selected':''}" onclick="answer('${m.id}','no')">Je ne veux / peux pas y aller</button></div>${deadline?`<div class="status">Réponse possible jusqu'au ${deadline.toLocaleString('fr-FR',{dateStyle:'short',timeStyle:'short'})}.</div>`:''}`:ticket?`<div class="status">✅ Billet pris · Tribune : ${esc(ticket.tribune||'Non renseignée')} · Prix : ${fmt(ticket.price)}</div>`:`<div class="status">⏳ Billet en attente</div>`}</div>`;}).join(''):'<div class="small">Aucun match ouvert pour le moment.</div>'}</div></div>`;
